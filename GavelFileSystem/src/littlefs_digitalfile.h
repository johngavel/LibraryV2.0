...
public:
  ...
  virtual const char* name() const override { return _display.c_str(); }
  ...
  virtual int size() override { return _file ? static_cast<int>(_file.size()) : 0; }
  virtual int read(unsigned char* buf, int __size) override {
    if (!_file) return -1;
    return static_cast<int>(_file.read(buf, static_cast<size_t>(__size)));
  }

  virtual operator bool() const override { return _file; }
  virtual bool isOpen() const override { return _file; }

  // --- Stream ---
  virtual int available() override { return _file ? _file.available() : 0; }
  virtual int read() override { return _file ? _file.read() : -1; }
  virtual int peek() override { return _file ? _file.peek() : -1; }
  ...
  virtual size_t write(unsigned char b) override {
    if (!_file) return 0;
...
public:
  ...
  virtual ~AliasDirectory() override {
  ...
  virtual void rewindDirectory() override { _cursor = 0; }
  ...
  virtual DigitalBase* getFile(const char* name) override { return findAlias(String(name)); }

  virtual bool reset() override {
    rewindDirectory();
    return true;
  }
  virtual void close() override { /* container: no-op */ }
